-- Fix notification to use proper user_id instead of customer_id
  ...
  IF NEW.status = 'delivered' 
    ...
    LOOP
      line_description := TRIM(
        COALESCE(order_item.product_name, '') ||
        CASE WHEN order_item.sku IS NOT NULL AND order_item.sku <> '' THEN ' (' || order_item.sku || ')' ELSE '' END
      );
      ...
      INSERT INTO public.wms_invoice_items (
        ...
        COALESCE(NULLIF(line_description, ''), 'Item'),
    ...
    -- Get the user_id from wms_customer_users for notification
    SELECT user_id INTO v_user_id
    FROM wms_customer_users
    WHERE customer_id = NEW.customer_id
    LIMIT 1;

    -- Only create notification if user_id found
